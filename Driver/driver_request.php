...
// Pre-fill from session if driver came from login page
$pre_name  = isset($_SESSION['req_driver_name'])  ? htmlspecialchars($_SESSION['req_driver_name'])  : '';
$pre_email = isset($_SESSION['req_driver_email']) ? htmlspecialchars($_SESSION['req_driver_email']) : '';
$pre_id    = isset($_SESSION['req_driver_id'])    ? (int)$_SESSION['req_driver_id']                 : 0;
...
if (isset($_POST['btnrequest'])) {
    ...
    if (!empty($errors)) {
        ...
        $safe_att    = $attachment_path ? mysqli_real_escape_string($con, $attachment_path) : 'NULL';
        $sid         = $sender_id > 0 ? $sender_id : 'NULL';
        $att_val     = $attachment_path ? "'$safe_att'" : 'NULL';
...
VALUES
('driver', $sid, '$safe_name', '$safe_email', '$safe_mobile',
 '$safe_subj', '$safe_msg', $att_val, 0)"
        ...
        if ($q) {
            ...
            try {
                ...
                $mail->Port       = 587;
                $mail->setFrom('carbook443@gmail.com', 'CarBook Driver Portal');
                $mail->addAddress($sender_email, $sender_name);
                $mail->isHTML(true);
                $mail->Subject = 'Your Driver Request Has Been Received — CarBook';
                $mail->Body    = "
                <div style='font-family:sans-serif;max-width:560px;margin:auto;padding:30px;background:#f8fafc;border-radius:12px;'>
                  <h2 style='color:#1e3a5f;'>Request Received</h2>
            ...
            unset($_SESSION['req_driver_name'], $_SESSION['req_driver_email'], $_SESSION['req_driver_id']);
...
if ($pre_id > 0) {
    $res = mysqli_query($con, "SELECT driver_mobile FROM driver_master WHERE driver_id = $pre_id");
    ...
    if ($row = mysqli_fetch_assoc($res)) {
        ...
        if (empty($driver_mobile)) {
            ...
            $modalMessage = 'Your mobile number is not found in database. Please contact admin.';
        }
    }
}
?>
...
<title>Driver Request — CarBook</title>
...
<main class="page">
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div class="notice">
        ...
        <p>Your account may be <strong>pending review</strong> or <strong>disabled</strong>. Use this form to contact our team. Provide as much detail as possible to speed up the process.</p>
      </div>
  ...
  <input type="text" id="subject" name="subject" class="form-input"
         value="Account disabled / rejected — appeal for re-activation" readonly>
</div>
    ...
    <div class="card-footer">
      Already approved? <a href="register.php">Sign in to your dashboard</a>
      &nbsp;·&nbsp;
...
if (form) {
  form.addEventListener('submit', function(e) {
    ...
    const mobile = document.getElementById('sender_mobile').value.trim();
  ...
  hasErr = true;
    ...
    if (setErr('subject',           'err-subject', !subj))                           hasErr = true;
